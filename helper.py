...
class ewriter():
    def __init__(self):
        ...
        self.PLAN_PROMPT = ("You are an expert writer tasked with writing a high level outline of an email to a therapist asking for an initial consultation. "
        ...
        self.WRITER_PROMPT = ("You are an writing assistant tasked with writing excellent emails. "
        ...
        self.RESEARCH_PLAN_PROMPT = ("You are a researcher charged with providing information that can "
                                     "be used when writing the following email requesting an intial consultation with a therapist or pychologist. Generate a list of search "
                                     "queries that will gather "
                                     "any relevant information on the best ways to structure the email. Only generate 3 queries max.")
        self.QUESTION_PROMPT =("You are a therapist that is tasked with asking one important question to a client that has just emailed you based on the recommendations provided here. "
        ...
        self.REFLECTION_PROMPT = ("You are a therapist's assistant deciding whether to reply to someone based on an email. "
        ...
        builder.add_node("question_node", self.question_node)
...
class writer_gui( ):
    ...
    def create_interface(self):
        with gr.Blocks(theme=gr.themes.Default(spacing_size='sm',text_size="sm")) as demo:
            ...
            with gr.Tab("Agent"):
                with gr.Row():
                    topic_bx = gr.Textbox(label="Prompt", value="Initial Consultation Email") 
                    gen_btn = gr.Button("Generate Email", scale=0,min_width=80, variant='primary')
                    cont_btn = gr.Button("Continue Email", scale=0,min_width=80)
                with gr.Row():
